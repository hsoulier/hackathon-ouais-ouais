...
const getData = async () => {
  try {
    ...
    data = (await response.json()) as CryptoList
    ...
    Object.values(data.data).map((cryptos, round) => {
                  ...
                  <article>
                      ...
                      <div class="article__information">
                        <div class="article__information__evolution">
                          <p class="priceCrypto">$${
                            cryptos.quotes.USD.price
                          }</p>
                          <p class="evolution">Évolution des dernières 24h :</p>
                          <span class="percent_change_24h">${
                            cryptos.quotes.USD.percent_change_24h > 0 ? "+" : ""
                          } ${cryptos.quotes.USD.percent_change_24h.toFixed(2)}%
                            ${
                              cryptos.quotes.USD.percent_change_24h > 0
                                ...
                                : '<svg clip-rule="evenodd" fill-rule="evenodd" stroke-linejoin="round" stroke-miterlimit="2" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="m2.005 12.002c0-5.517 4.48-9.997 9.997-9.997 5.518 0 9.998 4.48 9.998 9.997 0 5.518-4.48 9.998-9.998 9.998-5.517 0-9.997-4.48-9.997-9.998zm6.21 1.524s1.505 1.501 3.259 3.254c.147.147.338.22.53.22s.384-.073.531-.22c1.753-1.752 3.258-3.254 3.258-3.254.145-.145.217-.335.216-.526 0-.192-.074-.384-.22-.53-.293-.293-.766-.295-1.057-.004l-1.978 1.977v-6.693c0-.414-.336-.75-.75-.75s-.75.336-.75.75v6.693l-1.978-1.978c-.289-.289-.762-.287-1.055.006-.146.147-.22.339-.221.53s.071.38.215.525z" fill-rule="nonzero"/></svg>'
                            }
                          </span>
                        </div>
                      </div>
      ...
      let tabCryptoAccountPrice = Array.from(
        document.querySelectorAll(".price")
      )
      tabCryptoAccount.forEach((element, i) => {
        if (element.innerHTML === cryptos.name) {
          tabCryptoAccountPrice[i].textContent =
            cryptos.quotes.USD.price.toString()
...
let submitBtn = document.querySelector(
  ".crypto__virement form input[type=submit]"
)!
let moneyInput = document.querySelector<HTMLInputElement>(
  ".crypto__virement form input#money"
)!
let moneySelector = document.querySelector<HTMLInputElement>(
  ".crypto__virement form select#crypto"
)!

let cryptoWallet = document.querySelectorAll(
  ".crypto__myAccount table tr.wallet"
)!
  ...
  if (moneyInput.value != "" && moneySelector.value != "") {
    cryptoWallet.forEach((element) => {
      ...
      if (moneySelector.value == cryptoWalletName) {
        element.getElementsByTagName("td")[2].innerHTML = (
          parseFloat(element.getElementsByTagName("td")[2].innerHTML) -
          parseFloat(moneyInput.value)
        ).toString()
        total()
      }
    })
